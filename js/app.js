...
const App = {
  ...
  newContract() {
    ...
    this.isDirty = false;
    this.updateAutoSaveBadge('waiting');
  ...
  editContract(contractId) {
    try {
      ...
      this.isDirty = false;
      this.lastAutoSavedTime = new Date(contract.updatedAt || contract.createdAt || Date.now());
  ...
  startAutoSaveSystem() {
    ...
    // Periodic auto-save every 15 seconds while actively editing
    this.autoSaveInterval = setInterval(() => {
      if (this.currentView === 'editor' && this.isDirty && this.editingContract) {
        this.autoSaveContract(false);
      }
    }, 15000);
  },

  updateAutoSaveBadge(state, customMessage) {
    const badge = document.getElementById('editor-autosave-badge');
    if (!badge) return;

    badge.classList.remove('hidden', 'bg-amber-50', 'text-amber-800', 'border-amber-300', 'bg-blue-50', 'text-blue-800', 'border-blue-300', 'bg-emerald-50', 'text-emerald-800', 'border-emerald-300', 'bg-slate-100', 'text-slate-600', 'border-slate-300');

    if (state === 'dirty') {
      badge.classList.add('bg-amber-50', 'text-amber-800', 'border-amber-300');
      badge.innerHTML = `● Guardando cambios...`;
    } else if (state === 'saving') {
      badge.classList.add('bg-blue-50', 'text-blue-800', 'border-blue-300');
      badge.innerHTML = `⏳ Guardando automáticamente...`;
    } else if (state === 'saved') {
      const timeStr = this.lastAutoSavedTime ? this.lastAutoSavedTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '';
      badge.classList.add('bg-emerald-50', 'text-emerald-800', 'border-emerald-300');
      badge.innerHTML = `✓ Guardado automático ${timeStr ? '(' + timeStr + ')' : ''}`;
    } else if (state === 'waiting') {
      badge.classList.add('bg-slate-100', 'text-slate-600', 'border-slate-300');
      badge.innerHTML = `✍️ Escribe el nombre del cliente`;
  ...
  autoSaveContract(isUserAction = false) {
    if (!this.editingContract) return false;

    this.syncFormToModel();
    this.recalculateFormTotals();

    const clientName = (this.editingContract.client && this.editingContract.client.name) ? this.editingContract.client.name.trim() : '';

    if (!clientName) {
      try {
        localStorage.setItem('tk_contract_draft', JSON.stringify(this.editingContract));
      } catch (e) {}
      this.updateAutoSaveBadge('waiting');
      return false;
    }
    ...
    try {
      ...
      if (isUserAction) {
        this.showToast(`✓ Contrato #${saved.folio} guardado automáticamente`, 'success');
      }
      return true;
    } catch (err) {
      console.error('Error in autoSaveContract:', err);
      return false;
    }
  },
  ...
  onFormChange() {
    ...
    this.updateLivePreview();
    this.isDirty = true;
    this.updateAutoSaveBadge('dirty');

    // Debounce auto-save: 2 seconds after user stops typing
    if (this.autoSaveTimer) clearTimeout(this.autoSaveTimer);
    this.autoSaveTimer = setTimeout(() => {
      if (this.currentView === 'editor' && this.editingContract && this.isDirty) {
        this.autoSaveContract(false);
      }
    }, 2000);
  },
  ...
  saveCurrentContract(silent = false) {
    this.syncFormToModel();
    if (!this.editingContract.client || !this.editingContract.client.name || !this.editingContract.client.name.trim()) {
      if (!silent) {
  ...
  saveAndReturn() {
    const success = this.saveCurrentContract(false);
    if (success) {
      setTimeout(() => {
  ...
  viewAndPrintContract(contractId) {
    let contract = null;
    if (contractId) {
      if (this.editingContract && (this.editingContract.id === contractId || this.editingContract.folio === contractId)) {
        this.autoSaveContract(false);
    ...
    if (!contract) return;
    this.showToast('🖨️ Contrato guardado. Abriendo formato para imprimir...', 'info');
  ...
  async downloadContractPDF(contractId) {
    ...
    if (!contract) return;
    this.showToast('📥 Contrato guardado automáticamente. Generando PDF...', 'info');
    await PDFGenerator.downloadPDF(contract);
  },
